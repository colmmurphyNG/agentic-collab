  ...
  describe('destroyAgent — handoff snapshot (item Q)', () => {
    it('writes a handoff page at /pages/handoff-<name>-<ts>/ before kill_session', async () => {
      ...
      try {
        ...
        db.addDashboardMessage('handoff-agent', 'to_agent', 'work in progress on TICKET-NNNN', { topic: 'TICKET-NNNN' });
        ...
        assert.match(body, /work in progress on TICKET-NNNN/);
    ...
    it('mirrors handoff to project memory dir when CLAUDE_PROJECTS_DIR is set (item Q v2)', async () => {
      ...
      try {
        ...
        // convert: `/Users/test-user/dev/project-a` → `-Users-test-user-dev-project-a`.
        writeFileSync(
    ...
    it('appends to existing MEMORY.md instead of overwriting (item Q v2)', async () => {
      ...
      try {
        writeFileSync(
          ...
          '---\nengine: claude\ncwd: /Users/test-user/dev/project-d\n---\n',
        );
        writeFileSync(
        ...
        const memoryDir = join(projectsDir, '-Users-test-user-dev-project-d', 'memory');
    ...
    it('skips project-memory mirror silently when CLAUDE_PROJECTS_DIR is unset (item Q v2)', async () => {
      ...
      try {
        writeFileSync(join(personasDir, 'brain.md'), '---\nengine: claude\ncwd: /Users/test-user/dev/conductor\n---\n');

        db.createAgent({ name: 'brain', engine: 'claude', cwd: '/Users/test-user/dev/conductor', proxyId: 'p1' });
...
describe('claudeAddDirFlags — CLAUDE_ADD_DIRS env → --add-dir flags', () => {
  ...
  it('should emit a single --add-dir flag for one path', () => {
    process.env['CLAUDE_ADD_DIRS'] = '/Users/x/dev/project-b-worktrees';
    assert.equal(claudeAddDirFlags(), `--add-dir '/Users/x/dev/project-b-worktrees'`);
  });
