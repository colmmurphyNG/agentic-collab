...
export class DroneAuditAggregator {
  ...
  private slugToAgent(slug: string): string {
    // -Users-colm-murphy-dev-conductor → conductor; brain/tl etc. live there
    if (slug.endsWith('-conductor')) return 'conductor-agents';
    if (slug.includes('SFCC-Test-Suite')) return 'qa';
    if (slug.includes('SFCC-webapp')) return 'pwa';
    if (slug.includes('SFCC-commerce')) return 'sfcc';
    if (slug.includes('EcommerceAI')) return 'algo';
    if (slug.includes('Datadog')) return 'dd';
    if (slug === '-Users-colm-murphy-dev') return 'operator-dev-root';
    return slug.replace(/^-Users-colm-murphy-/, '').replace(/-/g, '/');
  }
}
