...
`);
  ...
  describe('engine_config resolution', () => {
    let db: Database;
    let personasDir: string;

    before(() => {
      personasDir = mkdtempSync(join(tmpdir(), 'persona-engine-config-'));
      db = new Database(join(personasDir, 'engine-config.db'));
      // Create an engine config that personas can reference
    ...
    it('resolves engine from engine_config when engine is missing', () => {
      writeFileSync(join(personasDir, 'config-agent.md'), [
        '---',
        'engine_config: claude-opus',
        'cwd: /tmp',
        '---',
        '# Agent using engine_config',
      ].join('\n'));

      const result = syncPersonasWithDiff(db, personasDir);
      assert.deepEqual(result.created, ['config-agent']);
      assert.deepEqual(result.skipped, []);

      const agent = db.getAgent('config-agent');
      assert.ok(agent);
      assert.equal(agent.engine, 'claude', 'engine should be resolved from engine_config');
      assert.equal(agent.engineConfig, 'claude-opus', 'engine_config should be stored');
    });

    it('skips persona with nonexistent engine_config', () => {
      writeFileSync(join(personasDir, 'bad-config.md'), [
        '---',
        'engine_config: nonexistent-config',
        'cwd: /tmp',
        '---',
        '# Agent with bad engine_config',
      ].join('\n'));

      const result = syncPersonasWithDiff(db, personasDir);
      assert.ok(result.skipped.includes('bad-config'), 'should be in skipped list');
      assert.ok(!db.getAgent('bad-config'), 'agent should not be created');
    });

    it('engine wins when both engine and engine_config are set', () => {
      writeFileSync(join(personasDir, 'both-agent.md'), [
        '---',
        'engine: codex',
        'engine_config: claude-opus',
        'cwd: /tmp',
        '---',
        '# Agent with both engine and engine_config',
      ].join('\n'));

      const result = syncPersonasWithDiff(db, personasDir);
      assert.ok(result.created.includes('both-agent'));

      const agent = db.getAgent('both-agent');
      assert.ok(agent);
      assert.equal(agent.engine, 'codex', 'direct engine should take priority over engine_config');
      assert.equal(agent.engineConfig, 'claude-opus', 'engine_config should still be stored');
    });
  });
